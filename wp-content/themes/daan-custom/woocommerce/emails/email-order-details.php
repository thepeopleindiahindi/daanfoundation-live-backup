...
<h2 class="<?php echo esc_attr( $heading_class ); ?>">
	...
	if ( $sent_to_admin ) {
		$order_number_string = __( '[Donation #%s]', 'woocommerce' );
	} else {
		$order_number_string = __( '[Donation Receipt #%s]', 'woocommerce' );
	}
...
<div style="margin-bottom: <?php echo $email_improvements_enabled ? '24px' : '40px'; ?>;">
	<table class="td font-family <?php echo esc_attr( $order_table_class ); ?>" cellspacing="0" cellpadding="6" style="width: 100%;" border="1">
		...
		<thead>
			<tr>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Cause', 'woocommerce' ); ?></th>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Amount', 'woocommerce' ); ?></th>
			</tr>
		</thead>
		<?php } ?>
		<tbody>
		...
		<tfoot>
			...
			if ( $item_totals ) {
				...
				foreach ( $item_totals as $total ) {
					...
					<tr class="order-totals order-totals-<?php echo esc_attr( $total['type'] ?? 'unknown' ); ?><?php echo esc_attr( $last_class ); ?>">
						<th class="td text-align-left" scope="row" colspan="1" style="<?php echo ( 1 === $i ) ? 'border-top-width: 4px;' : ''; ?>">
							<?php
			...
			if ( $order->get_customer_note() && ! $email_improvements_enabled ) {
				?>
				<tr>
					<th class="td text-align-left" scope="row" colspan="1"><?php esc_html_e( 'Note:', 'woocommerce' ); ?></th>
			...
			if ( $order->get_customer_note() && $email_improvements_enabled ) {
				...
				<tr class="order-customer-note">
					<td class="td text-align-left" colspan="2">
