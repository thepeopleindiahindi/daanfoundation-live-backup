...
<aside style="background: #fff; border-radius: 16px; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1), 0 8px 10px -6px rgba(0,0,0,0.1); padding: 24px; position: sticky; top: 104px;">
	...
	<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px;">
		<?php foreach ( array( 500, 1000, 2500, 5000 ) as $amt ) : ?>
			<button onclick="selectAmount(this, <?php echo $amt; ?>)" class="amount-btn <?php echo $amt === 1000 ? 'active' : ''; ?>" style="padding:10px 0;border-radius:10px;font-size:0.875rem;font-weight:700;cursor:pointer;transition:all 0.2s;background:<?php echo $amt === 1000 ? '#F97316' : 'transparent'; ?>;color:<?php echo $amt === 1000 ? '#fff' : '#F97316'; ?>;border:<?php echo $amt === 1000 ? 'none' : '2px solid #F97316'; ?>">₹<?php echo $amt; ?></button>
		<?php endforeach; ?>
	</div>
