    ...
    fields: [
      {
        ...
        placeholder: "0",
        helpText: "Include jewellery worn regularly",
      },
    ...
    fields: [
      {
        ...
        placeholder: "0",
        helpText: "Not your primary residence",
      },
...
export function ZakatCalculator() {
  ...
  const calculations = useMemo(() => {
    ...
    const totalLiabilities = liabilityFields.reduce((sum, field) => sum + getNum(field.id), 0);
    ...
    return { totalAssets, totalLiabilities, netAssets, nisabThreshold, isZakatDue, zakatPayable };
  }, [values, nisabType]);
    ...
    <div className="w-full max-w-3xl mx-auto">
          ...
          return (
            <div key={category.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              <button
                ...
                className="w-full flex items-center justify-between p-5 text-left hover:bg-[#F3F4F6] transition-colors"
              >
                <div>
                    ...
                    <div key={field.id}>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full rounded-xl border-2 border-slate-200 py-3 pl-8 pr-4 text-slate-900 placeholder:text-slate-300 focus:border-orange-400 focus:outline-none focus:ring-4 focus:ring-orange-400/10 transition-all"
                        />
                      </div>
      ...
      <div
        className={`rounded-2xl p-6 ${
          calculations.isZakatDue ? "bg-gradient-to-br from-orange-500 to-orange-600" : "bg-slate-100"
        }`}
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 text-sm font-medium transition-colors ${
              calculations.isZakatDue ? "text-white/80 hover:text-white" : "text-slate-500 hover:text-slate-700"
            }`}
          >
        ...
        <div className="space-y-3 mb-6">
          ...
          <div className={`border-t pt-3 ${calculations.isZakatDue ? "border-white/20" : "border-slate-200"}`}>
            <div className="flex justify-between">
              <span className={calculations.isZakatDue ? "text-white/80" : "text-slate-600"}>
        ...
        <div className={`rounded-xl p-4 ${calculations.isZakatDue ? "bg-white/10" : "bg-white"}`}>
          {calculations.isZakatDue ? (
            <>
              ...
              <button className="w-full rounded-xl bg-white py-3 text-base font-bold text-orange-500 hover:bg-orange-50 transition-colors">
                Pay Your Zakat Now
              </button>
            </>
          ) : (
            <div className="text-center py-2">
              <div className="text-lg font-semibold text-slate-900 mb-1">Zakat is not due</div>
              <div className="text-sm text-slate-600">Your net assets are below the Nisab threshold</div>
            </div>
          )}
        </div>
      </div>
